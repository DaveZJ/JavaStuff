public class Sudoku
{
	...
	public int NakedCol()
	{
		...
		for(int i = 0; i <= 8; i++)
		{
			...
			for(int j = 0; j<=8; j++)
			{
				lList[OpList[j][i].length()] = lList[OpList[j][i].length()+1]+j;
				for(int m = 2; m <= 8; m++)
				{	
			...
			for(int n = 2; n<= 8; n++)
			{
				for(int p = 0; p < lList[n]; p++)
				{
					String tempString = "";
					String rowString = "";
					for(int q = 0; q < llList[n-2].length(); q++)
					{
						if(OpList[(int)(lList[n].charAt(p))-48][i].length() > OpList[(int)(llList[n-2].charAt(q))-48][i].length())
						{
							int charCount = 0;
							int r = 0;
							int s = 0;
							while(r < OpList[(int)(llList[n-2].charAt(q))-48][i].length() && charCount == 0)
							{
								while((int)(OpList[(int)(lList[n].charAt(p))-48][i].charAt(s)) <= (int)(OpList[(int)(llList[n-2].charAt(q))-48][i].charAt(r)) && charCount == 0)
								{
									if((int)(OpList[(int)(lList[n].charAt(p))-48][i].charAt(s)) < (int)(OpList[(int)(llList[n-2].charAt(q))-48][i].charAt(r)) && (int)(OpList[(int)(lList[n].charAt(p))-48][i].charAt(s+1)) > (int)(OpList[(int)(llList[n-2].charAt(q))-48][i].charAt(r)))
									{
										charCount = 1;
									}
									s++;
							...
							if(charCount == 0)
							{
								tempString = tempString+(int)(lList[n-2].charAt(q));
							}
						}
						else
						{
							if(OpList[(int)(lList[n].charAt(p))-48][i] == OpList[(int)(lList[n-2].charAt(q))-48][i])
							{
								tempString = tempString+(int)(lList[n-2].charAt(q));
							}
						}
