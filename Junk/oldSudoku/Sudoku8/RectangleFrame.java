...
/* Clear Option Fields */
		...
		class forceButtonListener implements ActionListener
		{
			public void actionPerformed(ActionEvent event)
			{
				...
				int[] TheColArray = new int[mySudoku.SudokuSize()];
				int[] TheRowArray = new int[mySudoku.SudokuSize()];
				int RCcount = 0;
				for(int i = 0; i<= 8; i++)
				{
					for(int j = 0; j<= 8; j++)
					{	
						if((mySudoku.ReturnEntry(i,j)).length() == 0)
						{
							TheRowArray[RCcount] = i;
							TheColArray[RCcount] = j;
							RCcount++;
						}
					}
				}

				GARecursion myRecur = new GARecursion(0, mySudoku, mySudoku.SudokuSize(), TheColArray, TheRowArray, 0);
				int recurval = 1;
				while(myRecur.hasMoreConf() && 0 < recurval)
				{
