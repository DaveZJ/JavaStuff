...
/*
	...
	public void RowReduction(int a)
	{
		for(int j = 0; j <= 8; j++)
		{
			OpList[a][j] = "";
			for(int k = 1; k <= 9; k++)
			{
				if(SuList[a][j].length() == 0 && RowList[a][k-1] != 0 && ColList[j][k-1] != 0 && SqList[3*(int)(Math.floor(a/3))+(int)(Math.floor(j/3))][k-1] !=0)
				{
					OpList[a][j] = OpList[a][j] + k;
				}
			}
		}
	}

	public void ColReduction(int b)
	{
		for(int i = 0; i <= 8; i++)
		{
			OpList[i][b] = "";
			for(int k = 1; k <= 9; k++)
			{
				if(SuList[i][b].length() == 0 && RowList[i][k-1] != 0 && ColList[b][k-1] != 0 && SqList[3*(int)(Math.floor(i/3))+(int)(Math.floor(b/3))][k-1] !=0)
				{
					OpList[i][b] = OpList[i][b] + k;
				}
			}
		}
	}

	public void SqReduction(int c)
	{
		for(int i = 0; i <= 8; i++)
		{
			for(int j = 0; j <= 2; j++)
			{
				for(int l = 0; l <= 2; l++)
				{
					int TheRow = 3*(int)(Math.floor(c/3))+j;
					int TheCol = (l+3*c)%9;
					OpList[TheRow][TheCol] = "";
					for(int k = 1; k <= 9; k++)
					{
						if(SuList[TheRow][TheCol].length() == 0 && RowList[TheRow][k-1] != 0 && ColList[TheCol][k-1] != 0 && SqList[3*(int)(Math.floor(TheRow/3))+(int)(Math.floor(TheCol/3))][k-1] !=0)
						{
							OpList[TheRow][TheCol] = OpList[TheRow][TheCol] + k;
						}
					}
				}
			}
