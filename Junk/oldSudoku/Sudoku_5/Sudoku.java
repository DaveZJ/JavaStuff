public class Sudoku
{
	...
	public int HiddenCol()
	{
		...
		{	
			...
			{
				int q = 0;
				while(q < (OpList[j][i]).length())
				{
					IntCounter[(OpList[j][i]).charAt(q)-49] = IntCounter[(OpList[j][i]).charAt(q)-49] + 1;
					q++;
				}				
			}
			for(int n = 2; n<= 8; n++)
			{
				...
				for(int r= 1; r<= 9; r++)
				{
					if(IntCounter[r-1] <= n)
					{
						cellstring = cellstring + r;
					}
				}
				if(n <= cellstring.length())
				{
					OrdSet pSet = new OrdSet();
					pSet.fillSet(cellstring);
					String perString = pSet.PermSet(n);
					int b = 0;
					while(n*b < perString.length())
					{
						OrdSet substringSet = new OrdSet();
						substringSet.fillSet(perString.substring(n*b,n*(b+1)));
						int interCount = 0;
						for(int d = 0; d <= 8; d++)
						{
							if((substringSet.intersect(OpList[d][i])).length() > 0)
							{
								interCount++;
							}
						}
						if(interCount == n)
						{
							for(int t = 0; t <= 8; t++)
							{
								if(substringSet.isSubset(OpList[t][i]) == 0 && (substringSet.intersect(OpList[t][i])).length() > 0)
								{
									OpList[t][i] = substringSet.StringMinusSet(OpList[t][i]);
									stepcount = 1;
								}
							}
							if(stepcount == 1)
